...
function renderCueButtons() {
  ...
  cueStrip.innerHTML = QUICK_CUES.map(cue => `<button type="button" data-cue="${escapeAttribute(cue)}">${cue}</button>`).join("");
}

function renderPracticeList() {
  practiceList.innerHTML = PRACTICE_ITEMS.map(item => {
    const checked = state.practice[item.id] ? "checked" : "";
    return `
      <label class="practice-item">
        <input type="checkbox" data-practice-id="${item.id}" ${checked} />
